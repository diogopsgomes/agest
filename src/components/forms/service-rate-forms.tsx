...
import { getServiceRate, postServiceRate, putServiceRate } from '@/lib/api/services';
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from '@/components/ui/form';

const FormSchema = z.object({
	name: z.string({ required_error: 'O nome da tarifa de serviço é obrigatório' }).nonempty({ message: 'O nome da tarifa de serviço é obrigatório' }),
	price: z
		.string({
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem className="lg:col-span-4">
								...
								<FormControl>
									<Input className="text-sm" placeholder="Insira o nome da tarifa de serviço" {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormControl>
									<Input className="text-sm" placeholder="Insira o preço da tarifa de serviço" {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
...
export function EditServiceRateForm({ serviceRateId }: { serviceRateId: string }) {
	const form = useForm<z.infer<typeof FormSchema>>({
		resolver: zodResolver(FormSchema),
		defaultValues: {
			name: '',
