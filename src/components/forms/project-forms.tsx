...
import { getProjects, getProject, postProject, putProject } from '@/lib/api/projects';
...
const FormSchema = z.object({
	name: z.string({ required_error: 'O nome do projeto é obrigatório' }).nonempty({ message: 'O nome do projeto é obrigatório' }),
	description: z
		.string({
	...
	url: z.string({ required_error: 'O url do projeto é obrigatório' }).optional(),
	image_url: z.string({ required_error: 'O url da imagem do projeto é obrigatório' }).optional(),
	category: z.string({ required_error: 'A categoria do projeto é obrigatória' }).optional(),
});
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							<FormControl>
								<Input className="text-sm" placeholder="Insira o nome do projeto" {...field} />
							</FormControl>
							<FormMessage />
						</FormItem>
					)}
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							<FormControl>
								<Textarea className="text-sm min-h-30" placeholder="Insira a descrição do projeto" {...field} />
							</FormControl>
							<FormMessage />
						</FormItem>
					)}
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormControl>
									<Input className="text-sm" placeholder="Insira o URL do projeto" {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormControl>
									<Input className="text-sm" placeholder="Insira o URL da imagem do projeto" {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
