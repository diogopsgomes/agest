...
export default function ClientsTable() {
	...
	const filteredClients = clients.filter((client) => [client.name, client.company, client.vat, client.email, client.tlm, client.tlf].some((field) => field?.toLowerCase().includes(search.toLowerCase())));
				...
				<TableBody>
					{loading
						...
						: filteredClients.map((client, idx) => (
								<TableRow key={client.client_id} onClick={() => router.push(`/clientes/perfil/${client.client_id}`)} className={idx % 2 === 0 ? 'bg-muted/40 hover:bg-muted' : 'bg-muted/0 hover:bg-muted'}>
