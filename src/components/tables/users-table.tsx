...
export default function UsersTable() {
	...
	const filteredUsers = users.filter((user) => [user.name, user.email].some((field) => field?.toLowerCase().includes(search.toLowerCase())));
				...
				<TableBody>
					{loading
						...
						: filteredUsers.map((user, idx) => (
								<TableRow key={user.user_id} onClick={() => router.push(`/utilizadores/perfil/${user.user_id}`)} className={idx % 2 === 0 ? 'bg-muted/40 hover:bg-muted' : 'bg-muted/0 hover:bg-muted'}>
